...
USER_ROOT_CANDIDATES = ("/Users", "/C:/Users", "Users")
WINDOWS_ROOT_CANDIDATES = ("/Windows", "/C:/Windows", "Windows")
...
def iter_user_directories(handler, fs) -> list[dict]:
    ...
    for entry in list_dir(handler, fs, users_root):
        if not entry.is_dir or entry.name.lower() in SKIP_USERS:
            continue
        results.append({"username": entry.name, "path": entry.path, "inode": entry.inode})
    return results
...
def find_files(handler, fs, root_path: str, suffixes: Optional[Tuple[str, ...]] = None, names: Optional[Tuple[str, ...]] = None, max_depth: int = 4) -> list[dict]:
    ...
    def walk(path: str, depth: int) -> None:
        if depth > max_depth:
            return
        for entry in list_dir(handler, fs, root_path if depth == 0 else path):
            if entry.is_dir:
                walk(entry.path, depth + 1)
                continue
            lower_name = entry.name.lower()
            if suffixes and lower_name.endswith(suffixes):
                results.append({"name": entry.name, "path": entry.path, "inode": entry.inode, "size": entry.size})
                continue
            if exact_names and lower_name in exact_names:
                results.append({"name": entry.name, "path": entry.path, "inode": entry.inode, "size": entry.size})

    walk(normalize_tsk_path(root_path), 0)
    return results
