...
APPID_MAP = {
	...
	"46b106fe160f9bd": "Microsoft Visual Studio 2026",
...
def get_appid_name(appid_hex: str) -> str:
    return APPID_MAP.get(appid_hex.lower(), "Unknown")
...
def _list_dir(handler, fs, path: str):
    try:
        entries = handler.list_directory(fs, inode=None, path=path)
        logger.debug(f"[DIR] {path} → {len(entries)} entries")
        return entries
    except Exception as e:
        logger.error(f"[DIR ERROR] {path} → {e}")
        return []


def _find_users_root(handler, fs):
    for path in USER_PATH_CANDIDATES:
        entries = _list_dir(handler, fs, path)
        if entries:
            logger.info(f"[FOUND USERS DIR] {path}")
            return path
    logger.error("Users 디렉토리를 찾지 못함")
    return None
...
def collect_from_image(handler, fs) -> list[dict]:
    ...
    for user_entry in user_entries:
        ...
        for jl_type, ext in (
            ...
            logger.debug(f"[CHECK] {dir_path}")

            entries = _list_dir(handler, fs, dir_path)

            for e in entries:
                if e.is_dir:
                    continue

                if not e.name.lower().endswith(ext):
                    continue

                try:
                    raw = handler.read_file(fs, e.inode, max_bytes=20 * 1024 * 1024)
                except Exception as ex:
                    logger.error(f"[READ ERROR] {e.name} → {ex}")
                    continue
                ...
                try:
                    tmp = tempfile.NamedTemporaryFile(
                        suffix=f"_{e.name}", delete=False
                    )
                    tmp.write(raw)
                    tmp.close()
                except Exception as ex:
                    logger.error(f"[TMP ERROR] {e.name} → {ex}")
                    continue

                appid = e.name[: -len(ext)]

                results.append({
                    "username": username,
                    "appid": appid,
                    "appname": get_appid_name(appid),
                    "jl_type": jl_type,
                    "filename": e.name,
                    "tmp_path": tmp.name,
                ...
                logger.info(f"[JUMPLIST] {username} | {jl_type} | {e.name}")

    logger.info(f"Jumplist 수집 완료 – {len(results)}개")
    return results
...
def collect_from_directory(mount_root: str) -> list[dict]:
    ...
    users_root = None
    for path in users_root_candidates:
    ...
    for username in os.listdir(users_root):
        ...
        for jl_type, ext in (
            ...
            folder = os.path.join(
                ...
                "Windows", "Recent", jl_type,
            )

            if not os.path.isdir(folder):
                logger.debug(f"[SKIP] {folder}")
                continue
            ...
            for fname in os.listdir(folder):
                ...
                results.append({
                    ...
                    "jl_type": jl_type,
                    "filename": fname,
                    "tmp_path": os.path.join(folder, fname),
                    "collected_at": collected_at,
                })

                logger.info(f"[JUMPLIST] {username} | {jl_type} | {fname}")

    logger.info(f"Jumplist 수집 완료 (directory) – {len(results)}개")
    return results
